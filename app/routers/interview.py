...
from app.services.ai_model import (
    generate_questions_from_cover_letter,
    evaluate_interview,
)
...
async def create_interview_questions(
    ...
    interview_type: str = Form(
        ..., description="Interview type: real(실전면접), general(모의면접)"
    ),
    ...
    try:
        questions = generate_questions_from_cover_letter(
            cover_letter_content.decode("utf-8"), user_data
        )
...
async def evaluate_interview_request(
    ...
    try:
        ...
        evaluation_result = evaluate_interview(
            cover_letter_data, merged_input, user_data
        )
