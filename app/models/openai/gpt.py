...
openai_ef = OpenAIEmbeddingFunction(api_key=Config.OPENAI_API_KEY, model_name="text-embedding-3-small")


class ContentGenerator:
    cache = {}
    semantic_cache = chroma_client.create_collection(
        name="semantic_cache", embedding_function=openai_ef, metadata={"hnsw:space": "cosine"}
    )
        ...
        if cache_key not in self.cache:
            ...
            if similar_doc["documents"] and similar_doc["documents"][0] and similar_doc["distances"][0][0] < 0.2:
                ...
                self.semantic_cache.add(
                    documents=[cache_key], metadatas=[{"response": response_content}], ids=[cache_key]
                )
