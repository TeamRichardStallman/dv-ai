...
class QuestionGenerator(BaseGenerator):
    @traceable
    def generate_questions(self, questions_prompt: str, additional_context: str) -> QuestionsResponse:
...
class EvaluationGenerator(BaseGenerator):
    def __init__(self, user_data: EvaluationRequest):
        super().__init__()
        self.user_data = user_data

    @traceable
    def evaluate(self, evaluation_prompt: str, qa_input: str) -> Union[TechnicalEvaluationResponse, PersonalEvaluationResponse]:
        chat_prompt = ChatPromptTemplate.from_messages([
